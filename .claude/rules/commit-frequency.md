...
- 変更がある場合は**必ずコミットを提案**
- `/project-status` スキルでタスク提案時に自動チェック
...
2. 現在のブランチをチェック：
   - **Issue着手中**（`feature/*`, `bugfix/*`, `hotfix/*`）：提案しない
   - **通常作業**（`develop`, `main`）：提案する
3. `git status --short` で変更ファイル数をカウント
4. しきい値（3ファイル）を超えたら警告メッセージを表示
5. AIがユーザーにコミットを提案

**理由**: Issue作業中は複数ファイルを変更してからまとめてコミットすることが多いため、途中でコミット提案すると作業の妨げになる。develop/main での直接作業時のみ、こまめなコミットを推奨する。
