...
詳細は `.claude/rules/ci-cost-management.md` を参照。

**現状**: GitHub Actions の無料枠（2,000分/月）内で運用中

**監視方法**: ユーザーが「GitHub Actions の使用量を確認して」と依頼すると、AIが以下を実行：
1. 今月の実行履歴を取得（`gh run list`）
2. 実行時間を計算（開始時刻と終了時刻の差分）
3. 合計使用量と残り枠を報告
4. 1,800分を超えている場合は警告し、対策オプションを提示
