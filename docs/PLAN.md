# 瞑想・メモ書き習慣化アプリ 外部公開設計プラン

**最終更新**: 2026-02-03
**ステータス**: Phase 1 実装中

---

## プロジェクト概要

**目標**: 個人利用の瞑想・メモ書き習慣化アプリを外部公開し、日本中で使ってもらう

**方針**:
- ターゲット: 個人利用（瞑想・習慣化に興味がある個人）
- 収益モデル: フリーミアム（まずは基本機能を無料提供、将来的にプレミアム機能で収益化）
- ユーザー規模: 小規模（〜1,000人）でMVP検証
- デザイン: Apple風の高級感、使いやすさ・見やすさ重視、Core Web Vitals 最適化
- 広告収入は除外

---

## 1. データベース設計

### 1.1 主要テーブル

#### **users** - ユーザー管理
```sql
- id (UUID, PK)
- email (VARCHAR, UNIQUE)
- password_hash (VARCHAR, nullable)
- display_name (VARCHAR)
- auth_provider ('email' | 'google')
- plan_type ('free' | 'premium')
- timezone, locale
- created_at, updated_at
```

#### **sessions** - セッション記録
```sql
- id (UUID, PK)
- user_id (UUID, FK → users)
- type ('meditation' | 'journaling')
- duration (INTEGER, 秒数)
- pages_completed (INTEGER, nullable)
- completed_at (TIMESTAMP)
- INDEX: (user_id, completed_at DESC)
```

#### **user_settings** - ユーザー設定
```sql
- user_id (UUID, PK, FK → users)
- meditation_duration (INTEGER, [2,5,7,10,15])
- journaling_duration (INTEGER, [60,120,300,420,600])
- journaling_break_duration (INTEGER, [5,10,15])
- theme ('light' | 'dark' | 'system')
- notifications_enabled, sound_enabled (BOOLEAN)
```

#### **daily_stats** - 日次統計（集計用）
```sql
- id (UUID, PK)
- user_id (UUID, FK → users)
- stat_date (DATE)
- meditation_count, journaling_count (INTEGER)
- total_duration (INTEGER)
- UNIQUE: (user_id, stat_date)
```

#### **subscriptions** - サブスクリプション管理（将来用）
```sql
- id (UUID, PK)
- user_id (UUID, FK → users)
- plan_type ('premium_monthly' | 'premium_yearly')
- status ('active' | 'canceled' | 'expired')
- stripe_customer_id, stripe_subscription_id
- current_period_start, current_period_end
```

### 1.2 バリデーションルール

- **セッション**: duration 1〜3600秒、completed_at は未来日時不可
- **設定**: 選択肢は事前定義されたリストのみ
- **ユーザー**: メールアドレスはRFC 5322準拠、パスワードは12文字以上（大文字・数字・記号必須）
- **セキュリティ**: レート制限（ログイン5回/5分、API 100回/分）

---

## 2. 技術スタック

### 2.1 フロントエンド
- **Next.js 15** (App Router) + TypeScript
- **Tailwind CSS** + カスタムデザインシステム
- **React Query** (サーバー状態管理) - 将来的に追加
- **Zustand** (クライアント状態管理) - 将来的に追加

### 2.2 バックエンド
- **Next.js API Routes** (Serverless Functions)
- **Supabase** (PostgreSQL + 認証 + リアルタイム機能)
- **NextAuth.js v5** (認証: Email, Google)

### 2.3 インフラ（小規模MVP向け）
- **ホスティング**: Vercel
- **データベース**: Supabase (無料プラン → Pro $25/月)
- **メール送信**: Resend (認証メール、パスワードリセット)
- **モニタリング**: Vercel Analytics + Sentry
- **コスト試算**: $0〜$50/月（〜1,000ユーザー）

### 2.4 CI/CDパイプライン

**GitHub Actions** (品質チェック):
- テスト実行（Jest）
- Lint（ESLint）
- 型チェック（TypeScript）
- セキュリティスキャン（npm audit）
- ビルド検証

**Vercel** (自動デプロイ):
- GitHub Actionsが成功 → Vercelが自動ビルド・デプロイ
- PRごとにPreview URL生成
- mainブランチは本番環境に自動デプロイ

### 2.5 主要API設計

```
POST   /api/auth/signup          # 新規登録
POST   /api/auth/signin          # ログイン
GET    /api/sessions             # セッション一覧
POST   /api/sessions             # セッション作成
DELETE /api/sessions/:id         # セッション削除
GET    /api/sessions/stats       # 統計取得
GET    /api/settings             # 設定取得
PATCH  /api/settings             # 設定更新
```

---

## 3. デザインシステム

### 3.1 カラーパレット

**ライトモード**:
- Primary Purple: `#7C3AED` (瞑想)
- Primary Blue: `#2563EB` (メモ書き)
- Background: `#FFFFFF`, `#F9FAFB`, `#F3F4F6`
- Text: `#1F2937`, `#6B7280`, `#9CA3AF`
- Border: `#E5E7EB`

**ダークモード**:
- Primary Purple: `#A78BFA`
- Primary Blue: `#60A5FA`
- Background: `#0A0A0A`, `#1F1F1F`, `#2A2A2A`
- Text: `#F9FAFB`, `#D1D5DB`, `#9CA3AF`

### 3.2 タイポグラフィ

- フォント: `-apple-system, BlinkMacSystemFont, 'Segoe UI', 'Noto Sans JP', sans-serif`
- サイズ: 12px, 14px, 16px, 18px, 20px, 24px, 30px, 36px
- ウェイト: 400, 500, 600, 700

### 3.3 Core Web Vitals 最適化

- **LCP** (< 2.5s): next/image, next/font
- **FID** (< 100ms): コード分割
- **CLS** (< 0.1): 固定サイズ確保

---

## 4. プレミアム機能案（将来）

### 無料プラン
- 基本タイマー機能
- 過去30日間の履歴
- ローカルデータのみ

### プレミアムプラン（月額¥500 / 年額¥5,000）
1. マルチデバイス同期
2. 高度な分析・レポート
3. データエクスポート/インポート
4. カスタマイズ機能
5. リマインダー機能
6. ソーシャル機能

---

## 5. 実装マイルストーン

### Phase 1: MVP（2〜3ヶ月）

**Week 1-2: 環境構築・認証実装**
- ✅ Supabaseプロジェクト作成
- ✅ 依存関係インストール
- ✅ 環境変数設定
- 🔄 データベーススキーマ実装
- ⏳ NextAuth.js設定
- ⏳ ユーザー登録・ログイン機能

**Week 3-4: データ移行・API実装**
- ⏳ LocalStorageからDB移行ロジック
- ⏳ セッションCRUD API
- ⏳ 設定管理API
- ⏳ 統計計算API

**Week 5-6: UI改善・デザインシステム**
- ⏳ Apple風デザインシステム適用
- ⏳ ダークモード最適化
- ⏳ レスポンシブ対応強化

**Week 7-8: テスト・デプロイ**
- ⏳ E2Eテスト（Playwright）
- ⏳ パフォーマンステスト
- ⏳ Vercelへのデプロイ

### Phase 2: 正式リリース（1〜2ヶ月）
- ⏳ ベータテスト
- ⏳ ランディングページ
- ⏳ 正式リリース

### Phase 3: 拡張（3〜6ヶ月後）
- ⏳ プレミアム機能開発
- ⏳ Stripe決済統合

---

## 凡例
- ✅ 完了
- 🔄 実装中
- ⏳ 未着手
